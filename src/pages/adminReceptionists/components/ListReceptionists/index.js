            ...
            {receptionists.map((d, index) => (
                <Box key={index} className={styles.element} gridColumn="span 12" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    >
                        ...
                        <MenuItem onClick={() => { handleDeleteClick(d.username); handleClose(index)}} style={{ backgroundColor: 'rgba(255, 0, 0, 0.49)', color: 'white', borderRadius: '10px', marginLeft: '5px', marginRight: '5px' }}>Delete</MenuItem>
                    </Menu>
                    </div>
                </Box>
                ))}
